...
- **Auto-select the primary facet for big child-place queries, then fetch only that facet.** For a `child_place_type` query with `date="all"` and no explicit `source_override`:
  1. **Cheap probe:** fetch the same query at `date="latest"` (one obs per place per facet — small), rank facets with the existing ranking logic, pick the primary facet id.
  2. **Filtered fetch:** re-query with the original date but `source_ids=[primary]` → the API returns only that facet (~10× smaller), then the existing single-source path produces the result.
- **Refactor** the facet-ranking out of `_process_sources_and_filter_observations` into a reusable helper (`ByVariable → primary facet id`) used by both the probe and the existing full path (no duplicated ranking).
...
- **Source-selection fidelity (documented tradeoff):** the cheap `latest` probe cannot see total-observation-count (the 2nd tiebreaker), so when two sources **tie on place-coverage**, the auto-selected primary may differ from today's full-data ranking (it falls through to latest-date instead). Narrow; only affects coverage-ties. Perfect parity is impossible without the full multi-facet pull (the very thing we avoid). `source_override` (explicit) is always exact.

Non-goals: NO streaming/pagination (impossible on v2); NO place-sharding / beyond-series-cap support (that's A-ii); NO change to single-place, explicit-`source_override`, `date="latest"`, or `date=<single date>` queries; date-RANGE auto-reduction is out of scope for now (the `latest` probe's coverage may not match a range-filtered ranking — ranges keep current behavior, or use `source_override`).
